	...
	showScore: function (scr) {
		...
		for (var i=0; i<str.length; i++){
			...
			if (i<this.digSprites.length){
				...
				this.digSprites[i] = this.game.add.sprite(34*i,0,'PIC_DIG_FONT',9-k,this.content);
			//	this.digSprites[i] = this.game.add.sprite(this.x0+34*i,this.y0,'PIC_DIG_FONT',9-k,this.grp);
		...
		if (digsWidthChanged){
			...
			var scl = w/(34*str.length);
			if (scl<1){
				this.content.scale.set(w/(34*str.length));
			}
