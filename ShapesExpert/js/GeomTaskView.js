...
GeomTaskView.prototype = {
	init: function () {
		...
		this.shapeTxt=this.game.add.bitmapText (0, 0, 'font30', '0', 28);
		this.shapeTxt.align = 'left';
		this.shapeTxt.pivot.x = 0;//(this.shapeTxt.textWidth / 2);
		this.shapeTxt.pivot.y = (this.shapeTxt.textHeight / 2);
		this.content.add(this.shapeTxt);
		...
		this.okSmb = this.game.add.sprite(0,8,'PIC_RESULTS',0,this.content);
	...
	resetTask: function (phase, answer) {
		...
		if (answer) {
			...
			//this.brd.x = 0;
			this.okSmb.x = 0;
			this.shapeTxt.text = this.shapeText[this.shapeId];
