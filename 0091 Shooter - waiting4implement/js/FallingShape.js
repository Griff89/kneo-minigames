...
FallingShape.prototype = {
	...
	reset: function (id) {
		...
		this.geom.frame = id;//(Math.floor(Math.random() * 3)+ 5* Math.floor(Math.random() * 7));
		
		this.phase = id * Math.PI / 3;
		this.geom.x = 145 + 124 * id;
		this.moveMode = 1;
		this.planned2Fall = false;
	...
	doWaveMovement: function (dt) {
		this.phase += dt;
		this.geom.y = 212 + 20 * Math.sin(this.phase);
	...
	doFallMovement: function (dt) {
		this.fallTime += dt
		this.geom.y = 230 + 0.5 * this.g * this.fallTime * this.fallTime;
		if (this.geom.y >= 700) {
			this.showWrong();
			this.failCallBack();
		}
